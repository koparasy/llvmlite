...
LLVMPY_LLJIT_Link(std::shared_ptr<LLJIT> *lljit, const char *libraryName,
    ...
    //    if (!dylib) {
    //        char *message =
    //        LLVMGetErrorMessage(wrap(std::move(dylib.takeError()))); *OutError
    //        = LLVMPY_CreateString(message); LLVMDisposeErrorMessage(message);
    //        return nullptr;
    //    }
    //
    ...
    //    }
    //
    //    for (size_t element_idx = 0; element_idx < elements_length;
    //    element_idx++) {
    ...
    //                *dylib, ThreadSafeModule(std::move(module),
    //                std::move(ctxt)));
    //            if (addError) {
    //                char *message =
    //                LLVMGetErrorMessage(wrap(std::move(addError))); *OutError
    //                = LLVMPY_CreateString(message);
    //                LLVMDisposeErrorMessage(message);
    //                return nullptr;
    //            }
    //        }; break;
...
LLVMPY_LLJIT_Dylib_Tracker_Dispose(JITDylibTracker *tracker,
    ...
    *OutError = nullptr;
    auto result = false;
    /* This is undoubtedly a really bad and fragile
     * check. LLVM creates a bunch of platform
     * support to know if there's deinitializers to
     * run. If we try to run them when they aren't
     * present, a bunch of junk will be printed to
     * stderr. So, we're rummaging around for an
     * internal symbol in its platform support
     * library and skipping the deinitialization if
     * we don't find. */
